...
public class AngleShooter1 : MonoBehaviour {
	...
	void OnGUI(){
		windowRect = GUI.Window (15, new Rect ((Screen.width - box_w)/16, (Screen.height - box_h) - 70 , box_w, box_h), DoMyWindow3, "Angle (Angle : Medium)");
	...
	void DoMyWindow(int windowID) {
		...
		int text_h = 80;
		int text_w = 50;
		GUI.Label (new Rect (70, 40, 300, 70), "Amazing. You're really getting the hang of projectile motion now! Are you ready for the hardest Angle challenge?", myStyle);
		...
		if (GUI.Button (new Rect (235, 135, 150, 20), "Replay Medium")) {
			Application.LoadLevel ("AngleMedium");
		}
	}
	...
	void DoMyWindow3(int windowID) {
		...
		GUI.Label (new Rect (110, 80, 130, 30), "Velocity: 1223 Meters Per Second", myStyle);
	...
	IEnumerator TimerStart(){
		...
		rb.AddForce(transform.forward*1223);
