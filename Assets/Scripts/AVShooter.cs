...
public class AVShooter : MonoBehaviour {
	...
	void OnGUI(){
		windowRect = GUI.Window (4, new Rect ((Screen.width - box_w)/16, (Screen.height - box_h) - 70 , box_w, box_h), DoMyWindow3, "Variables (Angle-Velocity : Easy)");
	...
	void DoMyWindow(int windowID) {
		...
		int text_h = 80;
		int text_w = 50;
		GUI.Label (new Rect (70, 40, 300, 70), "You're really mastering projectile motion now! Get ready for a harder challenge!", myStyle);
		
		if (GUI.Button (new Rect (45, 135, 150, 20), "Medium Challenge")) {
			Application.LoadLevel("AVMedium");
		}
